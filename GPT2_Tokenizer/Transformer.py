...
class EncoderTransformerLayer(nn.Module): #EncoderLayer, post norm
    def __init__(self, embedding_size = 256, num_heads = 8, mlp_ratio = 4, bias = False, add_rope = False):
        super().__init__()
        self.embedding_size = embedding_size
        self.num_heads = num_heads
        self.rope = add_rope
        self.AttentionModule = MultiHeadAttention(input_emb = embedding_size, kq_emb = embedding_size, v_emb = embedding_size, num_heads = num_heads, bias = bias, add_rope=add_rope)
...
class EncoderTransformer(nn.Module):
    def __init__(self, num_layers = 4, embedding_size = 256, num_heads = 8, mlp_ratio = 4, bias = False, add_rope=False):
        super().__init__()
...
class DecoderTransformerLayer(nn.Module):#OG GPT2 post norm decoder layer- Note that masked multi head and then multi head and then FFN are being used
    def __init__(self, embedding_size = 256, num_heads = 8, is_causal = True, mlp_ratio = 4, bias = False, add_rope = False):
        super().__init__()
        self.embedding_size = embedding_size
        self.num_heads = num_heads
        self.rope = add_rope
        self.MaskedAttentionModule = MultiHeadAttention(input_emb = embedding_size, kq_emb = embedding_size, v_emb = embedding_size, num_heads = num_heads, bias = bias, add_rope=add_rope)
        self.LayerNorm1 = nn.LayerNorm(embedding_size)
        self.AttentionModule = MultiHeadAttention(input_emb = embedding_size, kq_emb = embedding_size, v_emb = embedding_size, num_heads = num_heads, bias = bias, add_rope=add_rope)
        self.LayerNorm2 = nn.LayerNorm(embedding_size)
        self.mlp_ratio = mlp_ratio
...
class CausalDecoderOnlyTransformer(nn.Module):
    def __init__(self, num_layers = 4, embedding_size = 256, num_heads = 8, mlp_ratio = 4, bias = False, add_rope = False):
        super().__init__()
        self.decoderLayers = nn.Sequential(*[CausalDecoderTransformerLayer(embedding_size = embedding_size, num_heads = num_heads, mlp_ratio = mlp_ratio, bias = bias, add_rope = add_rope)]*num_layers)
    def forward(self, x):
